...
subnet_pub_cidr=`grep SUBNET_PUBLIC_CIDR params.txt | cut -d "=" -f2 | sed 's/^\s\+//g;s/\s\+$//g'`
subnet_pub_az=`grep SUBNET_PUBLIC_AZ params.txt | cut -d "=" -f2 | sed 's/^\s\+//g;s/\s\+$//g'`
...
SUBNET_PUBLIC_ID=$(aws ec2 create-subnet \
  ...
  --availability-zone $subnet_pub_az \
  --query 'Subnet.{SubnetId:SubnetId}' \
  --output text \
  --region $aws_reg)
echo "Subnet ID '$SUBNET_PUBLIC_ID' CREATED in $subnet_pub_az Availability Zone."

# Add Name tag to Public Subnet
name $SUBNET_PUBLIC_ID subnet
...
inst_id=$(aws ec2 run-instances --image-id ami-0ac80df6eff0e70b5 --count $cnt --instance-type ${instance_type} --key-name $name --security-group-ids $GROUP_Id --subnet-id $SUBNET_PUBLIC_ID --output text | awk '{print $8}' | grep "^i-")
